...
public class RegAlloc {
	...
	for( int i = 0; i < block_order.size( ); i++ ){
	    BasicBlock b = block_order.get( i );
	    if( b.IsLoopHeader( ) ){
		loop_iterations.put( b , ( block_weights.get( b ) - 1 ) * 2 );
	    }
	}
	...
	*/
	...
	System.out.println("Calculating live intervals for " +  b.GetBlockId( ) );
	...
	for( int i = 0; i < b.NumPrev( ); i++ ){
		...
		while( b.HasMorePhi( ) ){
	        ...
	        System.out.print( "Updating " + prevblock.GetBlockId( ) + " " );
		...
		System.out.println( "current set = " + prev_live.set + " with " + temp_live.set );
